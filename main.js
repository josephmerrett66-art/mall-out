...
// ---------------------------------------------------------------------------
const QUALITY = {
  shadows: false, // off for smooth chunk streaming on normal laptops
  ...
  renderScale: 1.65, // >1 renders fewer pixels while CSS still fills the screen
...
scene.clearColor = new BABYLON.Color4(0.09, 0.085, 0.075, 1);
...
scene.fogColor = new BABYLON.Color3(0.42, 0.39, 0.34);
...
function addSharedSurfaceTextures() {
  ...
  const marbleBump = makeTexture("sharedMarbleBump", 256, (ctx, size) => {
    ctx.fillStyle = "#808080";
    ctx.fillRect(0, 0, size, size);
    for (let i = 0; i < 180; i++) {
      const v = 92 + Math.floor(Math.random() * 58);
      ctx.strokeStyle = `rgba(${v},${v},${v},0.22)`;
      ctx.lineWidth = 1 + Math.random() * 2;
      ctx.beginPath();
      const y = Math.random() * size;
      ctx.moveTo(-20, y);
      ctx.bezierCurveTo(size * 0.25, y + Math.random() * 50 - 25, size * 0.75, y + Math.random() * 50 - 25, size + 20, y + Math.random() * 40 - 20);
      ctx.stroke();
    }
  });
  // Keep the polished albedo texture, but avoid bump mapping on the endless
  // stacked floors; the underside of upper levels catches it too strongly.
  marbleBump.dispose();
...
materials.shopGlass.alpha = 0.18;
...
materials.floorSheen.alpha = 0.12;
...
materials.shadowMat.alpha = 0.16;
material("sunPatch", new BABYLON.Color3(0.95, 0.9, 0.72), 0.62);
materials.sunPatch.alpha = 0.12;
...
material("grime", new BABYLON.Color3(0.07, 0.065, 0.055), 0.92);
materials.grime.alpha = 0.035;
material("scuff", new BABYLON.Color3(0.42, 0.4, 0.36), 0.88);
materials.scuff.alpha = 0.035;
material("safetyYellow", new BABYLON.Color3(0.95, 0.72, 0.12), 0.5);
material("ventDark", new BABYLON.Color3(0.08, 0.085, 0.085), 0.74);
...
function refineMaterialRealism() {
  ...
  const alphaMode = BABYLON.Material.MATERIAL_ALPHABLEND ?? BABYLON.PBRMaterial.PBRMATERIAL_ALPHABLEND;
  for (const n of ["shopGlass", "railGlass", "skylight", "floorSheen", "shadowMat", "sunPatch", "water", "grime", "scuff"]) {
    if (!materials[n]) continue;
    materials[n].transparencyMode = alphaMode;
    materials[n].needDepthPrePass = false;
  }
}
...
const tileSize = 22;
const chunkTiles = 2;
const chunkSize = tileSize * chunkTiles;
const chunkRadius = 1;
const horizonRadius = chunkRadius + 3;
const unloadPadding = 0;
const nearDetailRadius = 0;
const maxChunkJobs = 12;
const chunkStepsPerUpdate = 1;
...
const vRadiusUp = 0; // only simulate the current floor; distant upper floors are faked visually
const vRadiusDown = 0;

const horizonRoot = new BABYLON.TransformNode("horizonArchitecture", scene);
let horizonKey = "";
const chunkJobs = [];
...
function chunkId(cx, cz, level) {
  return `${cx},${cz},${level}`;
}

function tileToChunk(tileX, tileZ, level) {
  return {
    cx: Math.floor(tileX / chunkTiles),
    cz: Math.floor(tileZ / chunkTiles),
    level,
  };
}
...
function positionToChunk(position) {
  return {
    ...
    level: Math.round(position.y / levelHeight),
  };
}

function getChunkEdits(key) {
  if (!chunkEdits.has(key)) chunkEdits.set(key, { removed: new Set(), builds: [] });
...
function createCylinder(name, options, position, mat, collides = true) {
  ...
  if (d > 0) options.tessellation = Math.max(options.tessellation || 0, roundSides(d, 10, 24));
...
function createSphere(name, options, position, mat, collides = false) {
  if (options.diameter) options.segments = Math.max(options.segments || 0, roundSides(options.diameter, 10, 24));
...
function createCapsule(name, options, position, mat, collides = false) {
  options.tessellation = options.tessellation || roundSides((options.radius || 0.4) * 2, 10, 20);
...
function makeBoxSignMaterial(key, lines, bg = "#1d2428", fg = "#f3e6bf") {
  const cacheKey = "boxsign|" + key + "|" + bg + "|" + fg;
  let mat = _labelMatCache.get(cacheKey);
  if (mat) return mat;
  const texture = new BABYLON.DynamicTexture("boxSignTexture", { width: 512, height: 192 }, scene);
  const ctx = texture.getContext();
  ctx.fillStyle = bg;
  ...
  const fontSize = textLines.length > 1 ? 38 : 48;
  ctx.font = `bold ${fontSize}px Arial`;
  textLines.forEach((line, index) => {
    ctx.fillText(line, 256, 96 + (index - (textLines.length - 1) / 2) * (fontSize + 8));
  });
  texture.update();
  mat = new BABYLON.StandardMaterial("boxSignMat", scene);
  mat.diffuseTexture = texture;
  mat.emissiveColor = new BABYLON.Color3(0.015, 0.012, 0.008);
  mat.backFaceCulling = false;
  _labelMatCache.set(cacheKey, mat);
  return mat;
}
...
function addPanelText(root, text, position, rotation = 0, width = 2.4, height = 0.9, bg = "#1d2428", fg = "#f3e6bf") {
  ...
  if (!mat) {
    ...
    mat.diffuseTexture = texture;
    mat.emissiveColor = new BABYLON.Color3(0.55, 0.47, 0.28);
    mat.backFaceCulling = false;
    _labelMatCache.set(key, mat);
  }
  plane.material = mat;
...
function addContactShadow(root, x, z, width, depth, rotation = 0, alphaScale = 1) {
  const patch = createBox("contactShadow", { width, height: 0.011, depth }, new BABYLON.Vector3(x, 0.071, z), materials.shadowMat, false);
  patch.rotation.y = rotation;
  patch.scaling.y = Math.max(0.4, alphaScale);
  root.push(patch);
}

function addFineFloorWear(root, x, z, rand, intensity = 1) {
  const count = Math.max(2, Math.round(2 + intensity));
  for (let i = 0; i < count; i++) {
    const a = rand * 11.7 + i * 1.91;
    const px = x + Math.cos(a * 1.3) * (1.4 + (i % 4) * 1.65);
    const pz = z + Math.sin(a * 0.9) * (1.3 + ((i + 1) % 4) * 1.45);
    const scuff = createBox(
      "floorScuff",
      { width: 0.28 + (i % 3) * 0.18, height: 0.008, depth: 0.026 + (i % 2) * 0.018 },
      new BABYLON.Vector3(px, 0.118 + i * 0.0005, pz),
      i % 4 === 0 ? materials.grime : materials.scuff,
      false,
    );
    scuff.rotation.y = a;
    root.push(scuff);
...
function addBaseboardWear(root, x, z, side = 1, rand = 0) {
  for (let i = -1; i <= 1; i++) {
    const smear = createBox(
      "baseboardWear",
      { width: 0.018, height: 0.07 + (i + 1) * 0.018, depth: 0.7 + ((i + 2) % 3) * 0.16 },
      new BABYLON.Vector3(x + side * 10.58, 0.62 + i * 0.04, z + i * 4.2 + rand * 0.3),
      materials.grime,
      false,
    );
    root.push(smear);
...
function addCeilingServiceDetails(root, x, z, y = 5.4, rand = 0) {
  const vent = createBox("ceilingVent", { width: 1.2, height: 0.035, depth: 0.58 }, new BABYLON.Vector3(x - 4.2, y - 0.305, z + 2.8), materials.ventDark, false);
  root.push(vent);
  for (let i = -2; i <= 2; i++) {
    root.push(createBox("ventSlat", { width: 0.045, height: 0.015, depth: 0.5 }, new BABYLON.Vector3(x - 4.2 + i * 0.2, y - 0.33, z + 2.8), materials.blackTrim, false));
  }
  const sprinkler = createCylinder("sprinklerHead", { diameter: 0.18, height: 0.04, tessellation: 12 }, new BABYLON.Vector3(x + 4.0 + rand * 0.4, y - 0.32, z - 2.6), materials.chrome, false);
  sprinkler.rotation.x = Math.PI / 2;
  root.push(sprinkler);
  root.push(createBox("ceilingAccessPanel", { width: 1.45, height: 0.018, depth: 1.0 }, new BABYLON.Vector3(x + 2.0, y - 0.335, z + 4.0), materials.ventDark, false));
}

function addGlassSmudges(root, x, z, rotation, width = 0.035, height = 0.7, depth = 0.9) {
  for (let i = -1; i <= 1; i++) {
    const smudge = createBox("glassSmudge", { width, height, depth: depth * (0.7 + i * 0.08) }, localPoint(x, z, rotation, i * 0.55, -0.235, 1.55 + i * 0.24), materials.floorSheen, false);
    smudge.rotation.y = rotation;
    smudge.rotation.z = 0.04 * i;
    root.push(smudge);
  }
}

function addFaceFrame(root, name, x, z, rotation, width, height, y, dz, mat = materials.blackTrim, rail = 0.055) {
  const top = createBox(`${name}Top`, { width, height: rail, depth: rail }, localPoint(x, z, rotation, 0, dz, y + height / 2), mat, false);
  const bottom = createBox(`${name}Bottom`, { width, height: rail, depth: rail }, localPoint(x, z, rotation, 0, dz, y - height / 2), mat, false);
  const left = createBox(`${name}Left`, { width: rail, height, depth: rail }, localPoint(x, z, rotation, -width / 2, dz, y), mat, false);
  const right = createBox(`${name}Right`, { width: rail, height, depth: rail }, localPoint(x, z, rotation, width / 2, dz, y), mat, false);
  for (const mesh of [top, bottom, left, right]) mesh.rotation.y = rotation;
  root.push(top, bottom, left, right);
}

function addVerticalCornerPosts(root, name, x, z, rotation, halfWidth, halfDepth, y, height, radius, mat = materials.blackTrim) {
  for (const sx of [-1, 1]) {
    for (const dz of [-1, 1]) {
      const post = createCylinder(name, { diameter: radius * 2, height, tessellation: 12 }, localPoint(x, z, rotation, sx * halfWidth, dz * halfDepth, y), mat, false);
...
function addLayeredBoxEdges(root, name, x, z, rotation, width, depth, y, mat = materials.blackTrim) {
  const front = createBox(`${name}FrontLip`, { width, height: 0.055, depth: 0.055 }, localPoint(x, z, rotation, 0, -depth / 2, y), mat, false);
  const back = createBox(`${name}BackLip`, { width, height: 0.055, depth: 0.055 }, localPoint(x, z, rotation, 0, depth / 2, y), mat, false);
  const left = createBox(`${name}LeftLip`, { width: 0.055, height: 0.055, depth }, localPoint(x, z, rotation, -width / 2, 0, y), mat, false);
  const right = createBox(`${name}RightLip`, { width: 0.055, height: 0.055, depth }, localPoint(x, z, rotation, width / 2, 0, y), mat, false);
  for (const mesh of [front, back, left, right]) mesh.rotation.y = rotation;
  root.push(front, back, left, right);
}
...
function addPlanter(root, x, z, scale = 1) {
  ...
  addVerticalCornerPosts(root, "planterRoundedCorner", x, z, 0, 1.13 * scale, 0.68 * scale, 0.33 * scale, 0.55 * scale, 0.045 * scale, materials.blackTrim);
  addLayeredBoxEdges(root, "planterLowerBevel", x, z, 0, 2.16 * scale, 1.26 * scale, 0.08 * scale, materials.blackTrim);
  ...
  addContactShadow(root, x, z, 2.2 * scale, 1.28 * scale, 0, 0.8);
}
...
function addTrashBin(root, x, z) {
  ...
  addContactShadow(root, x, z, 0.8, 0.8, 0, 0.75);
}
...
function addBench(root, x, z, rotation = 0) {
  ...
  for (let i = 0; i < 4; i++) {
    ...
    pieces.push(createCylinder("benchSlatEndCap", { diameter: 0.09, height: 0.16, tessellation: 10 }, localPoint(x, z, rotation, -1.72, -0.23 + i * 0.16, 0.58), materials.wood, false));
    pieces.push(createCylinder("benchSlatEndCap", { diameter: 0.09, height: 0.16, tessellation: 10 }, localPoint(x, z, rotation, 1.72, -0.23 + i * 0.16, 0.58), materials.wood, false));
  }
  for (let i = 0; i < 3; i++) {
    ...
    pieces.push(createBox("benchBackSlatLip", { width: 3.18, height: 0.025, depth: 0.13 }, localPoint(x, z, rotation, 0, -0.55, 0.92 + i * 0.18), materials.floorSheen, false));
  }
  ...
  root.push(...pieces);
  addContactShadow(root, x, z, 3.25, 0.95, rotation, 0.85);
}
...
function addDirectory(root, x, z, rotation = 0) {
  ...
  addFaceFrame(root, "directoryBezel", x, z, rotation, 1.95, 2.45, 1.85, -0.1, materials.chrome, 0.06);
  addLayeredBoxEdges(root, "directoryBaseBevel", x, z, rotation, 0.88, 0.52, 0.13, materials.blackTrim);
...
function addAdvertisingColumn(root, x, z) {
  ...
  root.push(createCylinder("adColumnInset", { diameter: 1.05, height: 2.35, tessellation: 18 }, new BABYLON.Vector3(x, 1.34, z), materials.shopGlass, false));
...
function addServiceDoor(root, x, z, rotation = 0) {
  ...
  addFaceFrame(root, "serviceDoorRaisedPanel", x, z, rotation, 1.16, 1.05, 1.45, -0.095, materials.blackTrim, 0.035);
  for (let i = -1; i <= 1; i++) {
...
function addCautionStand(root, x, z, rotation = 0) {
  ...
  addFacePlate(root, "cautionStandFoot", x, z, rotation, 0, -0.04, 0.18, 0.62, 0.055, materials.blackTrim, 0.018);
}
...
function addVendingMachine(root, x, z, rotation = 0, mat = materials.posterRed) {
  ...
  addVerticalCornerPosts(root, "vendingRoundedCorner", x, z, rotation, 0.52, 0.34, 1.08, 2.02, 0.035, materials.blackTrim);
  addLayeredBoxEdges(root, "vendingTopBevel", x, z, rotation, 1.08, 0.72, 2.15, materials.blackTrim);
  addLayeredBoxEdges(root, "vendingBaseBevel", x, z, rotation, 1.08, 0.72, 0.09, materials.blackTrim);
  addFaceFrame(root, "vendingGlassGasket", x, z, rotation, 0.78, 1.26, 1.32, -0.43, materials.rubber, 0.035);
...
function addATM(root, x, z, rotation = 0) {
  const body = createBox("atm", { width: 1.18, height: 1.65, depth: 0.78 }, new BABYLON.Vector3(x, 0.86, z), materials.directory, false);
  const hood = createBox("atmHood", { width: 1.28, height: 0.42, depth: 0.52 }, localPoint(x, z, rotation, 0, -0.18, 1.72), materials.blackTrim, false);
  ...
  for (const mesh of [body, hood, screen, keypad, card, receipt, plinth]) mesh.rotation.y = rotation;
  root.push(body, hood, screen, keypad, card, receipt, plinth);
  addVerticalCornerPosts(root, "atmRoundedCorner", x, z, rotation, 0.59, 0.38, 0.86, 1.54, 0.035, materials.blackTrim);
  addFaceFrame(root, "atmScreenBezel", x, z, rotation, 0.76, 0.5, 1.35, -0.46, materials.blackTrim, 0.04);
...
function addMaintenanceCart(root, x, z, rotation = 0) {
  ...
  const trayLip = createBox("cartTrayLip", { width: 1.32, height: 0.08, depth: 0.88 }, localPoint(x, z, rotation, 0, 0, 0.58), materials.blackTrim, false);
  ...
  for (const mesh of [base, trayLip, bucket, bucketRim, handle, mop]) mesh.rotation.y = rotation;
  wringer.rotation.y = rotation;
  handle.rotation.z = 0.35;
  mop.rotation.z = -0.42;
  root.push(base, trayLip, bucket, bucketRim, handle, mop, wringer);
  addVerticalCornerPosts(root, "cartRoundedCorner", x, z, rotation, 0.64, 0.4, 0.38, 0.34, 0.025, materials.blackTrim);
...
function addElevatorBank(root, x, z, rotation = 0) {
  ...
  for (const dx of [-0.62, 0.62]) {
    addFaceFrame(root, "elevatorDoorFrame", x, z, rotation, 1.08, 2.68, 1.45, -0.205, materials.chrome, 0.045);
    const last = root.slice(-4);
    for (const mesh of last) {
      mesh.position.x += dx * Math.cos(rotation);
      mesh.position.z += dx * Math.sin(rotation);
    }
  }
...
function addTrayReturn(root, x, z, rotation = 0) {
  ...
  addVerticalCornerPosts(root, "trayReturnRoundedCorner", x, z, rotation, 0.77, 0.31, 0.72, 1.18, 0.03, materials.chrome);
  addLayeredBoxEdges(root, "trayReturnTopLip", x, z, rotation, 1.5, 0.58, 1.38, materials.chrome);
  for (let i = 0; i < 4; i++) {
...
function addCondimentStation(root, x, z, rotation = 0) {
  ...
  addVerticalCornerPosts(root, "condimentRoundedCorner", x, z, rotation, 1.15, 0.36, 0.48, 0.82, 0.025, materials.chrome);
  addLayeredBoxEdges(root, "condimentCounterLip", x, z, rotation, 2.32, 0.74, 0.94, materials.chrome);
...
function addFoodCourtBulkhead(root, x, z, text = "FRESH FOOD") {
  ...
  root.push(createBox("foodCourtBulkheadCove", { width: 9.05, height: 0.08, depth: 0.08 }, new BABYLON.Vector3(x, 3.44, z - 0.24), materials.chrome, false));
  root.push(createBox("foodCourtBulkheadShadowGap", { width: 9.05, height: 0.06, depth: 0.06 }, new BABYLON.Vector3(x, 2.9, z - 0.31), materials.rubber, false));
...
function addSoftSeatingIsland(root, x, z, rotation = 0) {
  ...
  const cushionA = createCapsule("softSeatCushion", { radius: 0.17, height: 1.65, tessellation: 12 }, localPoint(x, z, rotation, -0.8, 0, 0.69), materials.posterCream, false);
  const cushionB = createCapsule("softSeatCushion", { radius: 0.17, height: 1.65, tessellation: 12 }, localPoint(x, z, rotation, 0.8, 1.0, 0.69), materials.posterCream, false);
  for (const mesh of [rug, seatA, seatB, cushionA, cushionB]) mesh.rotation.y = rotation;
  cushionA.rotation.z = Math.PI / 2;
  cushionB.rotation.z = Math.PI / 2;
  root.push(rug, seatA, seatB, cushionA, cushionB);
  addVerticalCornerPosts(root, "softSeatRoundedCorner", x, z, rotation, 1.65, 1.35, 0.35, 0.48, 0.04, materials.blackTrim);
...
function addStoreLightWall(root, x, z, direction, rand) {
  ...
  root.push(glowBar);
  for (let i = -1; i <= 1; i++) {
    root.push(createBox("storeBackPanelJoint", { width: 0.025, height: 2.0, depth: 0.035 }, new BABYLON.Vector3(x - direction * 0.055, 2.0, z + i * 1.55), materials.blackTrim, false));
  }
}
...
function addShopAwning(root, x, z, direction, rand) {
  ...
  for (let i = -2; i <= 2; i++) {
    root.push(createBox("awningSeam", { width: 0.018, height: 0.23, depth: 0.06 }, new BABYLON.Vector3(x - direction * 0.51, 3.27, z + i * 1.25), materials.blackTrim, false));
  }
}
...
function addUpperStorefrontRun(root, x, z, horizontal = true) {
  ...
  for (let i = -1; i <= 1; i++) {
    ...
    if (i !== 0) {
      const displayStand = createBox(
        "upperDisplayStand",
        horizontal ? { width: 0.52, height: 0.72, depth: 0.18 } : { width: 0.18, height: 0.72, depth: 0.52 },
        new BABYLON.Vector3(horizontal ? px + i * 0.7 : px, 5.72, horizontal ? pz : pz + i * 0.7),
        i === -1 ? materials.posterCream : materials.displayGold,
        false,
      );
      root.push(displayStand);
    }
...
function addScatteredMallClutter(root, x, z, rand, count = 4) {
  // Kept as a hook for future loot, but the mall art direction is clean:
  // no debris or apocalypse mess, just an empty shopping centre.
}

function addShoppingTrolley(root, x, z, rotation = 0) {
  const basket = createBox("shoppingTrolleyBasket", { width: 0.82, height: 0.46, depth: 1.05 }, new BABYLON.Vector3(x, 0.68, z), materials.shopGlass, false);
  basket.rotation.y = rotation;
  root.push(basket);
  addFaceFrame(root, "trolleyFront", x, z, rotation, 0.82, 0.42, 0.68, -0.54, materials.chrome, 0.035);
  addFaceFrame(root, "trolleyBack", x, z, rotation, 0.82, 0.42, 0.68, 0.54, materials.chrome, 0.035);
  for (let i = -1; i <= 1; i++) {
    const rail = createBox("trolleySideRail", { width: 0.035, height: 0.035, depth: 1.05 }, localPoint(x, z, rotation, i * 0.32, 0, 0.86), materials.chrome, false);
    rail.rotation.y = rotation;
    root.push(rail);
  }
  const handle = createCylinder("trolleyHandle", { diameter: 0.055, height: 0.9, tessellation: 10 }, localPoint(x, z, rotation, 0, 0.65, 0.98), materials.rubber, false);
  handle.rotation.z = Math.PI / 2;
  handle.rotation.y = rotation;
  root.push(handle);
  for (const sx of [-0.32, 0.32]) {
    for (const dz of [-0.35, 0.35]) {
      const wheel = createCylinder("trolleyWheel", { diameter: 0.13, height: 0.045, tessellation: 10 }, localPoint(x, z, rotation, sx, dz, 0.18), materials.rubber, false);
      wheel.rotation.z = Math.PI / 2;
      wheel.rotation.y = rotation;
      root.push(wheel);
    }
...
function addStorefrontReflections(root, x, z, direction, rand) {
  ...
  for (let i = -1; i <= 1; i += 2) {
    const handPrint = createBox("shopGlassSmear", { width: 0.018, height: 0.34, depth: 0.62 }, new BABYLON.Vector3(x - direction * 0.235, 1.18 + rand * 0.28, z + i * (1.1 + rand * 0.4)), materials.scuff, false);
    handPrint.rotation.z = -0.05 * i;
    root.push(handPrint);
  }
}
...
function addProductShelf(root, x, z, rotation = 0, colorMat = materials.posterBlue) {
  const shelf = createBox("productShelf", { width: 2.2, height: 1.48, depth: 0.42 }, new BABYLON.Vector3(x, 0.9, z), materials.blackTrim, false);
  shelf.rotation.y = rotation;
  root.push(shelf);
  addVerticalCornerPosts(root, "shelfRoundedUpright", x, z, rotation, 1.08, 0.21, 0.9, 1.5, 0.025, materials.chrome);
  addLayeredBoxEdges(root, "shelfTopLip", x, z, rotation, 2.2, 0.44, 1.66, materials.chrome);
  for (let row = 0; row < 3; row++) {
...
function districtInfo(tileX, tileZ) {
  const worldDistrictSize = districtSize * 2;
  const bx = Math.floor((tileX + 2) / worldDistrictSize);
  const bz = Math.floor((tileZ + 2) / worldDistrictSize);
  const localX = tileX - bx * worldDistrictSize;
  const localZ = tileZ - bz * worldDistrictSize;
  const roll = seeded(bx, bz);
  const corridorBias = seeded(tileX, bz * 17) > 0.26 || seeded(bx * 19, tileZ) > 0.36;
  const nearOrigin = bx === 0 && bz === 0 && Math.abs(tileX) <= 1 && Math.abs(tileZ) <= 1;
  const landmarkRoll = seeded(Math.floor(tileX / 4), Math.floor(tileZ / 4));
  const isLandmarkCore = landmarkRoll > 0.93 && Math.abs(tileX % 4) <= 1 && Math.abs(tileZ % 4) <= 1;
  const type = nearOrigin || isLandmarkCore ? "atrium" : roll > 0.86 ? "foodCourt" : !corridorBias && roll > 0.64 ? "atrium" : "concourse";
  const theme = districtTheme(tileX, tileZ);
  return { bx, bz, localX, localZ, roll, type, theme, isLandmarkCore };
}

const DISTRICT_THEMES = [
  { id: "fashion", label: "FASHION PRECINCT", floor: () => materials.tileAlt, sign: () => materials.posterBlue, light: () => materials.storeSoft },
  { id: "luxury", label: "LUXURY MALL", floor: () => materials.floorTrim, sign: () => materials.blackTrim, light: () => materials.displayGold },
  { id: "electronics", label: "TECH WALK", floor: () => materials.directory, sign: () => materials.posterRed, light: () => materials.displayCyan },
  { id: "food", label: "FOOD PRECINCT", floor: () => materials.posterCream, sign: () => materials.posterRed, light: () => materials.storeWarm },
  { id: "kids", label: "KIDS ARCADE", floor: () => materials.displayPink, sign: () => materials.warningYellow, light: () => materials.displayPink },
  { id: "home", label: "HOMEWARES", floor: () => materials.storeWarm, sign: () => materials.wood, light: () => materials.storeWarm },
  { id: "hardware", label: "HARDWARE MALL", floor: () => materials.shutter, sign: () => materials.warningYellow, light: () => materials.warmWallLight },
  { id: "cinema", label: "CINEMA COMPLEX", floor: () => materials.blackTrim, sign: () => materials.redNeon, light: () => materials.redNeon },
  { id: "furniture", label: "FURNITURE WAREHOUSE", floor: () => materials.table, sign: () => materials.awningGreen, light: () => materials.storeCool },
];

function districtTheme(tileX, tileZ) {
  const sx = Math.floor(tileX / 8);
  const sz = Math.floor(tileZ / 8);
  const a = Math.floor(seeded(sx, sz) * DISTRICT_THEMES.length) % DISTRICT_THEMES.length;
  const b = Math.floor(seeded(sx + 1, sz) * DISTRICT_THEMES.length) % DISTRICT_THEMES.length;
  const blend = (tileX / 8) - sx;
  return blend > 0.68 ? DISTRICT_THEMES[b] : DISTRICT_THEMES[a];
}
...
function addFloorPattern(root, x, z, district) {
  ...
  if ((Math.round(x / tileSize) + Math.round(z / tileSize)) % 2 === 0) {
    root.push(createBox("floorTone", { width: tileSize * 0.48, height: 0.02, depth: tileSize * 0.48 }, new BABYLON.Vector3(x - 4.5, 0.025, z + 4.5), district.theme.floor(), false));
  }
  ...
  if (district.theme.id === "luxury" || district.theme.id === "cinema") {
    stripeA.material = district.theme.sign();
    stripeB.material = district.theme.sign();
  }
  ...
  addFineFloorWear(root, x, z, seeded(Math.round(x / tileSize) + 41, Math.round(z / tileSize) - 17), district.type === "foodCourt" ? 1.5 : 1);
}
...
function addAtriumVoidFloor(root, x, z, hole = 11) {
  ...
  root.push(createBox("floorTrim", { width: 0.22, height: 0.04, depth: hole + 0.5 }, new BABYLON.Vector3(x + hole / 2, 0.04, z), materials.floorTrim, false));
  addTileGrout(root, x, z);
  addFineFloorWear(root, x - 3.2, z + 3.8, seeded(x / tileSize + 5, z / tileSize - 9), 0.8);
  addFineFloorWear(root, x + 3.5, z - 3.4, seeded(x / tileSize - 11, z / tileSize + 3), 0.8);
}
...
function addVoidRails(root, x, z, hole = 11, gap = 3.0) {
  ...
  function fullSide(d, cx, cz) {
    ...
    railBox("railFingerMarks", d === 0.1 ? W * 0.28 : 0.035, 0.28, d === 0.1 ? 0.035 : W * 0.28, cx + (d === 0.1 ? W * 0.18 : 0), 0.77, cz + (d === 0.1 ? 0 : W * 0.18), materials.floorSheen, false);
  }
  function gappedZSide(cz) {
    for (const sx of [-segOff, segOff]) {
      ...
      railBox("railFingerMarks", seg * 0.42, 0.22, 0.035, sx, 0.78, cz - 0.02, materials.floorSheen, false);
    }
  }
...
function addCeiling(root, x, z) {
  ...
  addCeilingServiceDetails(root, x, z, y, seeded(x / tileSize, z / tileSize));
}
...
function addChromeColumn(root, x, z, y = 3.5, height = 7) {
  ...
  addContactShadow(root, x, z, 1.25, 1.25, 0, 0.95);
  for (let i = -1; i <= 1; i += 2) {
    const streak = createBox("columnVerticalReflection", { width: 0.03, height: height * 0.68, depth: 0.035 }, new BABYLON.Vector3(x + i * 0.34, y + height * 0.02, z - 0.05), materials.floorSheen, false);
    root.push(streak);
  }
}
...
function addEscalator(root, x, z, rise = levelHeight, angle = 0) {
  ...
  for (const dz of [-0.86, run + 0.86]) {
    const safety = createBox("escalatorSafetyLine", { width: 2.05, height: 0.022, depth: 0.08 }, localPoint(x, z, angle, 0, dz, dz > 0 ? rise + 0.225 : 0.225), materials.safetyYellow, false);
    safety.rotation.y = angle;
    root.push(safety);
  }
  ...
  for (const side of [-1, 1]) {
    ...
    const skirt = createBox("escalatorSkirtPanel", { width: 0.07, height: 0.32, depth: slopeLen }, localPoint(x, z, angle, side * 1.04, halfFwd, rise / 2 + 0.18), materials.blackTrim, false);
    skirt.rotation.y = angle;
    skirt.rotation.x = -pitch;
    root.push(glass, hand, skirt);
  }
}
...
function addTableSet(root, x, z, rotation = 0) {
  ...
  root.push(createCylinder("tableUnderBevel", { diameterTop: 0.98, diameterBottom: 1.12, height: 0.08 }, new BABYLON.Vector3(x, 0.69, z), materials.blackTrim, false));
  addContactShadow(root, x, z, 2.8, 2.8, rotation, 0.65);
  for (let i = 0; i < 4; i++) {
    ...
    const seat = createCylinder("foodChair", { diameter: 0.58, height: 0.12, tessellation: 18 }, localPoint(x, z, f, 0, 1.0, 0.46), materials.seat, false);
    ...
    addFacePlate(root, "foodChairBackLip", x, z, f, 0, 1.18, 1.04, 0.48, 0.035, materials.posterCream, 0.012);
  }
}

const STORE_TYPES = [
  { id: "clothing", name: "TRENDZ", sub: "CLOTHING", mat: () => materials.posterBlue, accent: () => materials.posterRed },
  { id: "shoe", name: "SOLE CITY", sub: "SHOES", mat: () => materials.directory, accent: () => materials.posterCream },
  { id: "electronics", name: "ELECTRO MART", sub: "TECH", mat: () => materials.posterRed, accent: () => materials.displayCyan },
  { id: "supermarket", name: "FRESHWAY", sub: "SUPERMARKET", mat: () => materials.awningGreen, accent: () => materials.food },
  { id: "pharmacy", name: "CHEMIST", sub: "PHARMACY", mat: () => materials.storeCool, accent: () => materials.posterCream },
  { id: "hardware", name: "HARDWARE", sub: "TOOLS", mat: () => materials.shutter, accent: () => materials.warningYellow },
  { id: "camping", name: "OUTDOOR", sub: "CAMPING", mat: () => materials.awningGreen, accent: () => materials.wood },
  { id: "bookstore", name: "BOOK NOOK", sub: "BOOKS", mat: () => materials.wood, accent: () => materials.posterCream },
  { id: "furniture", name: "HOME LOFT", sub: "FURNITURE", mat: () => materials.storeWarm, accent: () => materials.seat },
  { id: "pet", name: "PET PLACE", sub: "SUPPLIES", mat: () => materials.awningBlue, accent: () => materials.displayGold },
  { id: "toy", name: "TOY VAULT", sub: "TOYS", mat: () => materials.displayPink, accent: () => materials.warningYellow },
  { id: "homewares", name: "HOMEMART", sub: "WARES", mat: () => materials.posterCream, accent: () => materials.storeSoft },
  { id: "jewellery", name: "LUMEN", sub: "JEWELLERY", mat: () => materials.blackTrim, accent: () => materials.displayGold },
  { id: "cafe", name: "COFFEE CO", sub: "CAFE", mat: () => materials.wood, accent: () => materials.storeWarm },
  { id: "foodcourt", name: "EAT STREET", sub: "FOOD", mat: () => materials.posterRed, accent: () => materials.food },
];

function getStoreType(tileX, tileZ, side) {
  return STORE_TYPES[Math.floor(seeded(tileX + side * 19, tileZ - side * 31) * STORE_TYPES.length) % STORE_TYPES.length];
}

function addStoreSign(root, store, frontX, z, frontRotation, direction) {
  const fascia = createBox("storeFascia", { width: 0.18, height: 0.62, depth: 6.9 }, new BABYLON.Vector3(frontX - direction * 0.055, 3.47, z), store.mat(), false);
  fascia.rotation.y = frontRotation;
  root.push(fascia);
  addPanelText(root, `${store.name}\n${store.sub}`, new BABYLON.Vector3(frontX - direction * 0.17, 3.47, z), frontRotation, 3.15, 0.52, "#171a1d", "#fff0c7");
  addFacePlate(root, "storeFasciaBottomLip", frontX, z, frontRotation, 0, -0.13, 3.14, 6.65, 0.055, materials.chrome, 0.018);
}

function addAisleShelves(root, x, z, rotation, colorMat = materials.posterCream, rows = 2) {
  for (let i = 0; i < rows; i++) {
    addProductShelf(root, x + i * 1.05 * Math.cos(rotation), z + i * 1.05 * Math.sin(rotation), rotation, colorMat);
  }
}

function addDisplayCounter(root, x, z, rotation, width = 2.0, mat = materials.table, glassTop = true) {
  const counter = createBox("storeCounter", { width, height: 0.78, depth: 0.72 }, new BABYLON.Vector3(x, 0.45, z), mat, false);
  const top = createBox("storeCounterTop", { width: width + 0.08, height: 0.08, depth: 0.82 }, localPoint(x, z, rotation, 0, 0, 0.88), glassTop ? materials.shopGlass : materials.chrome, false);
  const toe = createBox("storeCounterToe", { width: width + 0.04, height: 0.09, depth: 0.78 }, localPoint(x, z, rotation, 0, -0.01, 0.12), materials.blackTrim, false);
  for (const mesh of [counter, top, toe]) mesh.rotation.y = rotation;
  root.push(counter, top, toe);
  addLayeredBoxEdges(root, "counterLip", x, z, rotation, width + 0.08, 0.82, 0.93, materials.chrome);
}

function addLowProductRun(root, x, z, rotation, count, mat) {
  for (let i = 0; i < count; i++) {
    const item = createBox("displayProduct", { width: 0.32, height: 0.22 + (i % 2) * 0.12, depth: 0.28 }, localPoint(x, z, rotation, (i - (count - 1) / 2) * 0.45, 0, 1.14), i % 2 ? mat : materials.posterCream, false);
    item.rotation.y = rotation;
    root.push(item);
  }
}

function addShoeDisplay(root, x, z, rotation) {
  const table = createBox("shoeDisplayTable", { width: 2.4, height: 0.18, depth: 1.0 }, new BABYLON.Vector3(x, 0.72, z), materials.table, false);
  table.rotation.y = rotation;
  root.push(table);
  for (let i = -2; i <= 2; i++) {
    const shoe = createCapsule("displayShoe", { radius: 0.12, height: 0.38 }, localPoint(x, z, rotation, i * 0.42, -0.12, 0.92), i % 2 ? materials.blackTrim : materials.posterCream, false);
    shoe.rotation.y = rotation + Math.PI / 2;
    root.push(shoe);
  }
}

function addFurnitureDisplay(root, x, z, rotation) {
  const sofa = createBox("sofaBase", { width: 2.1, height: 0.45, depth: 0.85 }, new BABYLON.Vector3(x, 0.42, z), materials.seat, false);
  const back = createBox("sofaBack", { width: 2.1, height: 0.82, depth: 0.18 }, localPoint(x, z, rotation, 0, 0.44, 0.78), materials.seat, false);
  const armA = createBox("sofaArm", { width: 0.18, height: 0.62, depth: 0.86 }, localPoint(x, z, rotation, -1.12, 0, 0.55), materials.seat, false);
  const armB = createBox("sofaArm", { width: 0.18, height: 0.62, depth: 0.86 }, localPoint(x, z, rotation, 1.12, 0, 0.55), materials.seat, false);
  for (const mesh of [sofa, back, armA, armB]) mesh.rotation.y = rotation;
  root.push(sofa, back, armA, armB);
}

function addStoreInterior(root, store, centerX, backX, z, direction, rotation, rand) {
  const innerX = centerX - direction * 0.8;
  const rearX = backX - direction * 1.15;
  const accent = store.accent();

  if (store.id === "clothing") {
    addMannequin(root, innerX, z - 2.1, rotation);
    addMannequin(root, innerX, z + 2.1, rotation);
    addStoreFixtureCluster(root, rearX, z, rotation, accent);
    addProductShelf(root, rearX, z + 3.0, Math.PI / 2, accent);
  } else if (store.id === "shoe") {
    addShoeDisplay(root, innerX, z, rotation);
    addProductShelf(root, rearX, z - 2.9, Math.PI / 2, accent);
    addProductShelf(root, rearX, z + 2.9, Math.PI / 2, materials.blackTrim);
  } else if (store.id === "electronics") {
    addDisplayCounter(root, innerX, z - 1.6, rotation, 2.4, materials.directory, true);
    addDisplayCounter(root, innerX, z + 1.6, rotation, 2.4, materials.directory, true);
    for (let i = -2; i <= 2; i++) addFacePlate(root, "screenWall", rearX, z, rotation, i * 0.62, -0.1, 1.85, 0.42, 0.32, materials.screen, 0.018);
  } else if (store.id === "jewellery") {
    addDisplayCounter(root, innerX, z - 1.7, rotation, 2.1, materials.blackTrim, true);
    addDisplayCounter(root, innerX, z + 1.7, rotation, 2.1, materials.blackTrim, true);
    addLowProductRun(root, innerX, z - 1.7, rotation, 4, materials.displayGold);
    addLowProductRun(root, innerX, z + 1.7, rotation, 4, materials.displayGold);
  } else if (store.id === "furniture") {
    addFurnitureDisplay(root, innerX, z - 1.6, rotation);
    addDisplayCounter(root, rearX, z + 2.2, rotation, 1.4, materials.wood, false);
    addSoftSeatingIsland(root, rearX, z - 2.0, rotation);
  } else if (store.id === "cafe" || store.id === "foodcourt") {
    addDisplayCounter(root, innerX, z - 2.2, rotation, 2.6, materials.wood, false);
    addDisplayCounter(root, rearX, z + 1.8, rotation, 2.8, materials.table, true);
    addTableSet(root, innerX, z + 2.2, rotation);
  } else if (store.id === "bookstore") {
    addAisleShelves(root, innerX, z - 2.2, Math.PI / 2, materials.wood, 2);
    addAisleShelves(root, innerX, z + 1.6, Math.PI / 2, materials.posterCream, 2);
    addDisplayCounter(root, rearX, z, rotation, 2.0, materials.wood, false);
  } else if (store.id === "camping") {
    addProductShelf(root, innerX, z - 2.8, Math.PI / 2, accent);
    addProductShelf(root, rearX, z + 2.8, Math.PI / 2, materials.awningGreen);
    const tent = createBox("displayTent", { width: 1.9, height: 0.86, depth: 1.4 }, new BABYLON.Vector3(innerX, 0.55, z + 0.8), materials.awningGreen, false);
    tent.rotation.y = rotation;
    tent.rotation.z = 0.12;
    root.push(tent);
  } else {
    addProductShelf(root, innerX, z - 2.6, Math.PI / 2, accent);
    addProductShelf(root, innerX, z + 2.6, Math.PI / 2, materials.posterCream);
    addAisleShelves(root, rearX, z, Math.PI / 2, accent, store.id === "supermarket" || store.id === "hardware" ? 3 : 2);
    addDisplayCounter(root, centerX - direction * 2.4, z + 0.2, rotation, 1.6, store.id === "pharmacy" ? materials.storeCool : materials.table, true);
  }

  if (rand > 0.54) addLowProductRun(root, centerX - direction * 1.2, z - 3.65, rotation, 5, accent);
  if (rand > 0.62) addLowProductRun(root, centerX - direction * 1.2, z + 3.65, rotation, 5, materials.posterCream);
}

function addDistrictLandmark(root, x, z, district, rand) {
  const choice = Math.floor(seeded(district.bx + 71, district.bz - 43) * 7);
  addHangingWayfinder(root, x, z - 7.8, 0, district.theme.label);
  if (choice === 0) {
    addReflectingPool(root, x, z, 0);
    addHangingLight(root, x, z, new BABYLON.Color3(0.78, 0.92, 1));
  } else if (choice === 1) {
    addSoftSeatingIsland(root, x, z, rand * Math.PI);
    addAdvertisingColumn(root, x + 5.2, z - 3.4);
  } else if (choice === 2) {
    addEscalatorLanding(root, x - 2.7, z - 4.6, 0);
    addEscalator(root, x + 2.3, z - 6.0, levelHeight, 0);
  } else if (choice === 3) {
    const stage = createBox("eventStage", { width: 6.6, height: 0.55, depth: 3.6 }, new BABYLON.Vector3(x, 0.32, z), materials.blackTrim, false);
    const backdrop = createBox("eventBackdrop", { width: 6.8, height: 2.8, depth: 0.18 }, new BABYLON.Vector3(x, 1.68, z + 2.0), district.theme.sign(), false);
    root.push(stage, backdrop);
    addPanelText(root, "CENTRE\nEVENTS", new BABYLON.Vector3(x, 2.08, z + 1.88), 0, 3.4, 0.9, "#151719", "#fff0bd");
  } else if (choice === 4) {
    for (let i = -2; i <= 2; i++) addPlanter(root, x + i * 2.2, z + Math.sin(i) * 1.2, 0.72);
    addDirectory(root, x - 5.8, z + 4.4, -Math.PI / 6);
  } else if (choice === 5) {
    addAnchorStorePortal(root, x, z + 7.6, 0, district.theme.id === "cinema" ? "CINEMA" : "DEPARTMENT STORE");
    addAnchorStorePortal(root, x, z - 7.6, Math.PI, district.theme.id === "hardware" ? "MEGA HARDWARE" : "ANCHOR STORE");
  } else {
    const displayBase = createCylinder("seasonalDisplayBase", { diameter: 4.8, height: 0.24, tessellation: 32 }, new BABYLON.Vector3(x, 0.18, z), materials.posterCream, false);
    root.push(displayBase);
    for (let i = 0; i < 9; i++) {
      const box = createBox("seasonalDisplay", { width: 0.52, height: 0.5 + (i % 3) * 0.22, depth: 0.52 }, new BABYLON.Vector3(x + Math.cos(i) * 1.55, 0.55, z + Math.sin(i) * 1.55), i % 2 ? materials.posterRed : materials.displayGold, false);
      box.rotation.y = i;
      root.push(box);
    }
  }
...
function buildShop(tileX, tileZ, side, rand, root) {
  ...
  const store = getStoreType(tileX, tileZ, side);
  const frontX = x + direction * 3.45;
  const backX = x + direction * 10.45;
  const centerX = (frontX + backX) / 2;
  const storeDepth = Math.abs(backX - frontX);
  ...
  const shopFloor = createBox("shopFloor", { width: storeDepth, height: 0.12, depth: 9.5 }, new BABYLON.Vector3(centerX, 0.08, z), materials.floor);
  const soffit = createBox("shopSoffit", { width: storeDepth + 0.25, height: 0.45, depth: 9.8 }, new BABYLON.Vector3(centerX, 4.25, z), materials.blackTrim, false);
  const sideA = createBox("shopSideWall", { width: storeDepth, height: 4.2, depth: 0.24 }, new BABYLON.Vector3(centerX, 2.1, z - 4.85), materials.shopDark);
  const sideB = createBox("shopSideWall", { width: storeDepth, height: 4.2, depth: 0.24 }, new BABYLON.Vector3(centerX, 2.1, z + 4.85), materials.shopDark);
  root.push(backWall, shopFloor, soffit, sideA, sideB);
  root.push(createBox("shopSoffitLowerReveal", { width: storeDepth + 0.35, height: 0.07, depth: 9.9 }, new BABYLON.Vector3(centerX, 3.98, z), materials.rubber, false));
  root.push(createBox("shopBackWallBaseCurve", { width: 0.12, height: 0.32, depth: 9.45 }, new BABYLON.Vector3(backX - direction * 0.25, 0.36, z), materials.blackTrim, false));
  ...
  root.push(glass);
  addGlassSmudges(root, frontX, z, frontRotation, 0.018, 0.62, 0.7);
  ...
  root.push(createBox("shopThresholdDirt", { width: 0.08, height: 0.012, depth: 5.8 }, new BABYLON.Vector3(frontX - direction * 0.48, 0.145, z), materials.grime, false));
  ...
  addFacePlate(root, "slidingDoorPanelA", frontX, z, frontRotation, -0.42, -0.23, 1.62, 0.72, 2.28, materials.shopGlass, 0.018);
  addFacePlate(root, "slidingDoorPanelB", frontX, z, frontRotation, 0.42, -0.245, 1.62, 0.72, 2.28, materials.shopGlass, 0.018);
  addFacePlate(root, "automaticDoorSensor", frontX, z, frontRotation, 0, -0.25, 2.93, 0.5, 0.12, materials.screen, 0.018);
  addFacePlate(root, "doorTrack", frontX, z, frontRotation, 0, -0.255, 2.88, 2.05, 0.055, materials.chrome, 0.018);
  root.push(createCylinder("shopDoorPull", { diameter: 0.055, height: 0.72, tessellation: 10 }, new BABYLON.Vector3(frontX - direction * 0.24, 1.42, z + 1.14), materials.chrome, false));
  ...
  addStoreSign(root, store, frontX, z, frontRotation, direction);
  ...
  if (rand > 0.84) {
    ...
    addStoreInterior(root, store, centerX, backX, z, direction, frontRotation, rand);
    if (rand > 0.72) addPartialShutter(root, frontX - direction * 0.12, z - 0.6, direction);
  }
...
function buildConcourse(root, tileX, tileZ, rand, district) {
  const x = tileX * tileSize;
  const z = tileZ * tileSize;
  ...
  addBaseboardWear(root, x, z - 1.5, -1, rand);
  addBaseboardWear(root, x, z + 1.5, 1, rand);
  ...
  if (rand > 0.42) {
    ...
    const stockBox = createBox("kioskStockBox", { width: 0.72, height: 0.42, depth: 0.54 }, new BABYLON.Vector3(x + 1.1, 1.12, z - 0.3), materials.posterCream, false);
    addInteractable(stockBox, "loot", "Scavenge kiosk", { scrap: 2, food: rand > 0.76 ? 1 : 0 });
    root.push(kiosk, stockBox);
  }
  ...
  if (rand > 0.62) addHangingWayfinder(root, x, z, 0, rand > 0.88 ? district.theme.label : rand > 0.74 ? "FOOD COURT" : "SHOPS");
  ...
  if (rand > 0.55) addShoppingTrolley(root, x - 2.8, z + 2.6, rand * Math.PI * 2);
  if (rand > 0.2) addScatteredMallClutter(root, x, z + 1.5, rand, rand > 0.75 ? 5 : 3);
  if (rand > 0.52) addFloorShadow(root, x, z + 4.5, 5.5, 1.4, 0.12);
  if (district.isLandmarkCore) addDistrictLandmark(root, x, z, district, rand);
}
...
function buildAtrium(root, tileX, tileZ, district, rand) {
  ...
  if (rand > 0.58) addShoppingTrolley(root, x + 8.5, z + 7.1, -Math.PI / 6);
...
function buildFoodCourt(root, tileX, tileZ, district, rand) {
  ...
  if (Math.abs(district.localX) === 2 || Math.abs(district.localZ) === 2) {
    ...
    if (rand > 0.56) addShoppingTrolley(root, x - 3.5, z + 3.2, Math.PI / 3);
...
function buildTile(root, tileX, tileZ) {
  const x = tileX * tileSize;
  const z = tileZ * tileSize;
  ...
  if (district.type === "concourse") {
    addCeiling(root, x, z);
    buildConcourse(root, tileX, tileZ, rand, district);
  }
...
// Performance-first mall grammar used by live chunk streaming. The older
// high-detail builders remain available for future hero rooms, but the active
// world uses these larger architectural pieces so chunks stay cheap.
function addFastStoreInterior(root, store, x, z, direction, frontRotation, rand) {
  const accent = store.accent();
  const storeX = x + direction * 6.8;
  const rearX = x + direction * 9.1;
  const displayX = x + direction * 4.8;

  if (store.id === "clothing") {
    for (const dz of [-1.7, 1.7]) {
      root.push(createBox("fastClothesRack", { width: 1.4, height: 1.15, depth: 0.18 }, new BABYLON.Vector3(displayX, 0.78, z + dz), accent, false));
      root.push(createBox("fastMannequin", { width: 0.42, height: 1.45, depth: 0.32 }, new BABYLON.Vector3(rearX, 0.82, z + dz * 0.75), materials.mannequin, false));
    }
  } else if (store.id === "electronics") {
    root.push(createBox("fastScreenWall", { width: 0.12, height: 1.7, depth: 4.6 }, new BABYLON.Vector3(rearX, 1.55, z), materials.screen, false));
    for (const dz of [-1.8, 0, 1.8]) root.push(createBox("fastDeviceTable", { width: 1.35, height: 0.58, depth: 0.75 }, new BABYLON.Vector3(storeX, 0.4, z + dz), materials.directory, false));
  } else if (store.id === "bookstore") {
    for (const dz of [-2.6, -0.9, 0.9, 2.6]) root.push(createBox("fastBookcase", { width: 0.72, height: 1.8, depth: 0.42 }, new BABYLON.Vector3(rearX, 1.0, z + dz), materials.wood, false));
    root.push(createBox("fastBookTable", { width: 1.8, height: 0.44, depth: 1.1 }, new BABYLON.Vector3(displayX, 0.32, z), materials.posterCream, false));
  } else if (store.id === "cafe" || store.id === "foodcourt") {
    root.push(createBox("fastCafeCounter", { width: 1.0, height: 0.9, depth: 4.8 }, new BABYLON.Vector3(rearX, 0.55, z), materials.wood, false));
    for (const dz of [-1.6, 1.6]) root.push(createCylinder("fastCafeTable", { diameter: 0.78, height: 0.12, tessellation: 10 }, new BABYLON.Vector3(displayX, 0.66, z + dz), materials.table, false));
  } else if (store.id === "furniture" || store.id === "homewares") {
    root.push(createBox("fastSofa", { width: 1.8, height: 0.55, depth: 0.9 }, new BABYLON.Vector3(displayX, 0.4, z - 1.6), materials.seat, false));
    root.push(createBox("fastDisplayCabinet", { width: 0.72, height: 1.65, depth: 3.6 }, new BABYLON.Vector3(rearX, 0.9, z + 1.2), materials.table, false));
  } else if (store.id === "camping") {
    const tent = createBox("fastTent", { width: 1.8, height: 0.9, depth: 1.4 }, new BABYLON.Vector3(displayX, 0.55, z), materials.awningGreen, false);
    tent.rotation.z = direction * 0.12;
    root.push(tent);
    root.push(createBox("fastOutdoorShelf", { width: 0.82, height: 1.6, depth: 4.4 }, new BABYLON.Vector3(rearX, 0.9, z), materials.wood, false));
  } else {
    for (const dz of [-2.2, 0, 2.2]) {
      root.push(createBox("fastProductShelf", { width: 0.72, height: 1.55, depth: 1.25 }, new BABYLON.Vector3(rearX, 0.86, z + dz), accent, false));
    }
    root.push(createBox("fastCounter", { width: 1.4, height: 0.72, depth: 0.82 }, new BABYLON.Vector3(storeX, 0.42, z + (rand > 0.5 ? 2.6 : -2.6)), materials.table, false));
  }

  root.push(createBox("fastBackRoomDoor", { width: 0.08, height: 1.85, depth: 1.05 }, new BABYLON.Vector3(rearX + direction * 0.18, 1.05, z - 3.6), materials.serviceDoor, false));
  root.push(createBox("fastInteriorLight", { width: 2.8, height: 0.045, depth: 0.28 }, new BABYLON.Vector3(storeX, 3.55, z), materials.light, false));
}

function buildFastShop(root, tileX, tileZ, side, rand) {
  const x = tileX * tileSize;
  const z = tileZ * tileSize;
  const direction = side === -1 ? -1 : 1;
  const rotation = side === -1 ? Math.PI / 2 : -Math.PI / 2;
  const store = getStoreType(tileX, tileZ, side);
  const frontX = x + direction * 3.35;
  const backX = x + direction * 10.45;
  const centerX = (frontX + backX) / 2;
  const fasciaMat = makeBoxSignMaterial(store.id, [store.name, store.sub], "#1a1d20", "#ffe8b0");

  root.push(createBox("fastShopShell", { width: Math.abs(backX - frontX), height: 3.8, depth: 9.6 }, new BABYLON.Vector3(centerX, 1.95, z), materials.shopDark));
  root.push(createBox("fastShopFloor", { width: Math.abs(backX - frontX), height: 0.08, depth: 9.4 }, new BABYLON.Vector3(centerX, 0.08, z), materials.floor, false));
  root.push(createBox("fastShopGlass", { width: 0.08, height: 2.45, depth: 6.8 }, new BABYLON.Vector3(frontX, 1.58, z), materials.shopGlass, false));
  root.push(createBox("fastShopFascia", { width: 0.16, height: 0.72, depth: 7.3 }, new BABYLON.Vector3(frontX - direction * 0.06, 3.42, z), fasciaMat, false));
  root.push(createBox("fastInteriorGlowWall", { width: 0.1, height: 2.1, depth: 5.8 }, new BABYLON.Vector3(backX - direction * 0.35, 1.75, z), rand > 0.5 ? materials.storeWarm : materials.storeCool, false));
  root.push(createBox("fastShopHeader", { width: 0.18, height: 0.18, depth: 7.3 }, new BABYLON.Vector3(frontX - direction * 0.02, 3.02, z), materials.blackTrim, false));
  root.push(createBox("fastShopKick", { width: 0.16, height: 0.3, depth: 7.3 }, new BABYLON.Vector3(frontX, 0.3, z), materials.blackTrim, false));
  for (const dz of [-3.35, -1.12, 1.12, 3.35]) root.push(createBox("fastMullion", { width: 0.1, height: 2.45, depth: 0.1 }, new BABYLON.Vector3(frontX - direction * 0.04, 1.55, z + dz), materials.blackTrim, false));
  for (const dz of [-3.8, 3.8]) {
    root.push(createBox("fastBladeSignMount", { width: 0.08, height: 0.55, depth: 0.08 }, new BABYLON.Vector3(frontX - direction * 0.2, 3.04, z + dz), materials.blackTrim, false));
    root.push(createBox("fastBladeSign", { width: 1.75, height: 0.42, depth: 0.08 }, new BABYLON.Vector3(frontX - direction * 0.62, 3.05, z + dz), makeBoxSignMaterial(`${store.id}-blade`, store.name, "#f1e4c1", "#151515"), false));
  }
  addFacePlate(root, "fastDoorTrack", frontX, z, rotation, 0, -0.18, 2.78, 1.6, 0.055, materials.chrome, 0.014);
  addFacePlate(root, "fastSalePoster", frontX, z, rotation, direction > 0 ? -2.0 : 2.0, -0.15, 1.22, 0.68, 0.95, rand > 0.5 ? materials.posterRed : materials.posterCream, 0.014);
  addFastStoreInterior(root, store, x, z, direction, rotation, rand);
}

function buildFastConcourse(root, tileX, tileZ) {
  const x = tileX * tileSize;
  const z = tileZ * tileSize;
  const rand = seeded(tileX, tileZ);
  const district = districtInfo(tileX, tileZ);
  const wide = district.isLandmarkCore || rand > 0.78;
  const corridorHalf = wide ? 4.7 : 3.45;

  root.push(createBox("floor", { width: tileSize, height: 0.32, depth: tileSize }, new BABYLON.Vector3(x, -0.16, z), materials.marble));
  root.push(createBox("fastFloorBand", { width: corridorHalf * 1.6, height: 0.018, depth: tileSize }, new BABYLON.Vector3(x, 0.018, z), district.theme.floor(), false));
  root.push(createBox("fastCentreLine", { width: 0.16, height: 0.022, depth: tileSize }, new BABYLON.Vector3(x, 0.035, z), materials.floorTrim, false));
  root.push(createBox("fastCeiling", { width: tileSize, height: 0.22, depth: tileSize }, new BABYLON.Vector3(x, 5.3, z), materials.whiteWall, false));
  root.push(createBox("fastCeilingCove", { width: corridorHalf * 2.1, height: 0.18, depth: tileSize }, new BABYLON.Vector3(x, 5.05, z), materials.ceiling, false));
  for (const dz of [-6.3, 0, 6.3]) root.push(createBox("fastLightPanel", { width: 3.4, height: 0.045, depth: 0.34 }, new BABYLON.Vector3(x, 4.88, z + dz), materials.light, false));

  buildFastShop(root, tileX, tileZ, -1, rand);
  buildFastShop(root, tileX, tileZ, 1, 1 - rand);

  if (rand > 0.52) {
    const kz = z + (rand > 0.5 ? 5.3 : -5.3);
    addBench(root, x + (rand > 0.5 ? 2.4 : -2.4), kz, rand > 0.5 ? 0 : Math.PI);
  }
  if (rand > 0.62) addPlanter(root, x + (rand > 0.5 ? 2.8 : -2.8), z + (rand > 0.5 ? -4.2 : 4.2), 0.72);
  if (rand > 0.78) addDirectory(root, x, z - 6.2, 0);
  for (const dz of [-7.6, 0, 7.6]) {
    root.push(createBox("fastOverheadDirectory", { width: 3.8, height: 1.05, depth: 0.1 }, new BABYLON.Vector3(x, 3.18, z + dz), makeBoxSignMaterial("overhead-a", ["Food Court  ↑", "Restrooms  ↑", "Parking  →"], "#a99f8b", "#080808"), false));
  }
  if (rand > 0.66) addHangingWayfinder(root, x, z - 1.6, 0, district.theme.label);
  if (district.isLandmarkCore) {
    root.push(createCylinder("fastLandmarkMedallion", { diameter: 5.6, height: 0.04, tessellation: 24 }, new BABYLON.Vector3(x, 0.08, z), materials.floorTrim, false));
    addHangingWayfinder(root, x, z - 7.4, 0, "CENTRE COURT");
  }
}

function buildMidTile(root, tileX, tileZ) {
  const x = tileX * tileSize;
  const z = tileZ * tileSize;
  const rand = seeded(tileX, tileZ);
  const district = districtInfo(tileX, tileZ);
  root.push(createBox("floor", { width: tileSize, height: 0.35, depth: tileSize }, new BABYLON.Vector3(x, -0.18, z), materials.marble));
  root.push(createBox("midCeiling", { width: tileSize, height: 0.22, depth: tileSize }, new BABYLON.Vector3(x, 5.35, z), materials.ceiling, false));
  root.push(createBox("midWallL", { width: 0.45, height: 5.0, depth: tileSize }, new BABYLON.Vector3(x - 11, 2.5, z), materials.whiteWall));
  root.push(createBox("midWallR", { width: 0.45, height: 5.0, depth: tileSize }, new BABYLON.Vector3(x + 11, 2.5, z), materials.whiteWall));
  root.push(createBox("midFloorTrimA", { width: tileSize, height: 0.025, depth: 0.22 }, new BABYLON.Vector3(x, 0.025, z), district.theme.floor(), false));
  root.push(createBox("midFloorTrimB", { width: 0.22, height: 0.026, depth: tileSize }, new BABYLON.Vector3(x, 0.026, z), materials.floorTrim, false));

  for (const side of [-1, 1]) {
    const store = getStoreType(tileX, tileZ, side);
    const frontX = x + side * 3.5;
    const backX = x + side * 10.35;
    const centerX = (frontX + backX) / 2;
    const rot = side === -1 ? Math.PI / 2 : -Math.PI / 2;
    const signMat = makeBoxSignMaterial(`mid-${store.id}`, [store.name, store.sub], "#1a1d20", "#ffe8b0");
    root.push(createBox("midShopMass", { width: Math.abs(backX - frontX), height: 3.9, depth: 9.4 }, new BABYLON.Vector3(centerX, 1.98, z), materials.shopDark));
    root.push(createBox("midShopGlass", { width: 0.08, height: 2.45, depth: 6.2 }, new BABYLON.Vector3(frontX, 1.66, z), materials.shopGlass, false));
    root.push(createBox("midShopFascia", { width: 0.14, height: 0.5, depth: 6.7 }, new BABYLON.Vector3(frontX - side * 0.05, 3.38, z), signMat, false));
    root.push(createBox("midShopHeader", { width: 0.16, height: 0.16, depth: 6.7 }, new BABYLON.Vector3(frontX, 3.05, z), materials.blackTrim, false));
    root.push(createBox("midShopMullion", { width: 0.1, height: 2.4, depth: 0.1 }, new BABYLON.Vector3(frontX - side * 0.03, 1.66, z - 2.6), materials.blackTrim, false));
    root.push(createBox("midShopMullion", { width: 0.1, height: 2.4, depth: 0.1 }, new BABYLON.Vector3(frontX - side * 0.03, 1.66, z + 2.6), materials.blackTrim, false));
    if (rand > 0.72) {
      const fakeShelf = createBox("midInteriorShelves", { width: 0.12, height: 1.3, depth: 3.8 }, new BABYLON.Vector3(frontX - side * 1.25, 0.82, z), materials.posterCream, false);
      fakeShelf.rotation.y = rot;
      root.push(fakeShelf);
    }
  }

  for (let i = -1; i <= 1; i++) {
    root.push(createBox("midDownlight", { width: 1.2, height: 0.04, depth: 0.22 }, new BABYLON.Vector3(x + i * 5.4, 5.05, z - 5.2), materials.light, false));
    root.push(createBox("midDownlight", { width: 1.2, height: 0.04, depth: 0.22 }, new BABYLON.Vector3(x + i * 5.4, 5.05, z + 5.2), materials.light, false));
  }
  root.push(createBox("midOverheadDirectory", { width: 3.6, height: 0.96, depth: 0.1 }, new BABYLON.Vector3(x, 3.1, z), makeBoxSignMaterial("overhead-mid", ["Food Court  ↑", "Restrooms  ↑", "Parking  →"], "#a99f8b", "#080808"), false));
  if (rand > 0.78) addHangingWayfinder(root, x, z, 0, district.theme.label);
}

function objectKeyFor(mesh) {
  const p = mesh.getAbsolutePosition ? mesh.getAbsolutePosition() : mesh.position;
  return `${mesh.name}:${Math.round(p.x * 10)}:${Math.round(p.y * 10)}:${Math.round(p.z * 10)}`;
}

function rehydrateBuild(root, build, levelNode) {
  if (build.type === "barricade") {
    const wall = createBox("barricade", { width: 3.8, height: 2.2, depth: 0.45 }, new BABYLON.Vector3(build.x, build.y, build.z), materials.barricade);
    wall.rotation.y = build.rotation;
    wall.parent = levelNode;
    root.push(wall);
    placed.push(wall);
  }
  if (build.type === "camp") {
    const camp = createCylinder("camp", { diameter: 1.3, height: 0.45 }, new BABYLON.Vector3(build.x, build.y, build.z), materials.camp, false);
    const light = new BABYLON.PointLight("campLight", new BABYLON.Vector3(build.x, build.y + 1.2, build.z), scene);
    light.diffuse = new BABYLON.Color3(1, 0.42, 0.16);
    light.intensity = 1.4;
    light.range = 8;
    camp.parent = levelNode;
    light.parent = levelNode;
    root.push(camp, light);
    placed.push(camp, light);
  }
}

function queueChunk(cx, cz, level, lod) {
  const key = chunkId(cx, cz, level);
  const existing = mall.get(key);
  if (existing && existing._lod === lod) return;
  if (existing && existing._lod === "near") return;
  if (queuedChunks.has(key)) return;
  if (chunkJobs.length >= maxChunkJobs) return;
  if (existing && lod === "near") {
    unloadChunk(key, existing);
  } else if (existing) {
    return;
  }
  const levelNode = new BABYLON.TransformNode(`chunk_${key}`, scene);
  levelNode.position.y = level * levelHeight;
  levelNode._isLevelNode = true;
  const tiles = [];
  const startTileX = cx * chunkTiles;
  const startTileZ = cz * chunkTiles;
  for (let dx = 0; dx < chunkTiles; dx++) {
    for (let dz = 0; dz < chunkTiles; dz++) {
      tiles.push({ tileX: startTileX + dx, tileZ: startTileZ + dz });
    }
  }
  chunkJobs.push({ key, cx, cz, level, lod, root: [], levelNode, tiles, step: 0 });
  queuedChunks.add(key);
  console.info(`[mall] queued ${lod} chunk ${key}`);
}

function completeChunkJob(job) {
  const edits = getChunkEdits(job.key);
  for (const item of job.root) {
    if (item && item.parent === null) item.parent = job.levelNode;
  }
  for (let i = job.root.length - 1; i >= 0; i--) {
    const item = job.root[i];
    if (!(item instanceof BABYLON.Mesh) || !interactables.has(item)) continue;
    const objectKey = objectKeyFor(item);
    item.metadata = { ...(item.metadata || {}), chunkKey: job.key, objectKey };
    if (edits.removed.has(objectKey)) {
      interactables.delete(item);
      item.dispose();
      job.root.splice(i, 1);
    }
  }
  for (const build of edits.builds) rehydrateBuild(job.root, build, job.levelNode);
  job.root.push(job.levelNode);
  job.root._lod = job.lod;

  finalizeTile(job.root);
  mall.set(job.key, job.root);
  queuedChunks.delete(job.key);
  console.info(`[mall] created ${job.lod} chunk ${job.key} (${job.root.length} objects)`);
}

function processChunkJobs() {
  let steps = 0;
  while (chunkJobs.length && steps < chunkStepsPerUpdate) {
    const job = chunkJobs[0];
    const tile = job.tiles[job.step];
    if (tile) {
      if (job.lod === "near") buildFastConcourse(job.root, tile.tileX, tile.tileZ);
      else buildMidTile(job.root, tile.tileX, tile.tileZ);
      job.step++;
      steps++;
      continue;
    }
    completeChunkJob(job);
    chunkJobs.shift();
  }
}

function pruneChunkJobs(cx, cz, tl) {
  for (let i = chunkJobs.length - 1; i >= 0; i--) {
    const job = chunkJobs[i];
    const stale =
      Math.abs(job.cx - cx) > chunkRadius + unloadPadding ||
      Math.abs(job.cz - cz) > chunkRadius + unloadPadding ||
      job.level !== tl;
    if (!stale) continue;
    queuedChunks.delete(job.key);
    job.root.forEach((item) => item.dispose?.());
    job.levelNode.dispose(true);
    chunkJobs.splice(i, 1);
    console.info(`[mall] cancelled stale chunk ${job.key}`);
  }
}

function unloadChunk(key, items) {
  items.forEach((item) => {
    if (item._isLevelNode) return;
    interactables.delete(item);
  ...
  console.info(`[mall] unloaded chunk ${key}`);
}

const GEN_BUDGET = 5; // max chunk jobs queued per update

function acquireHorizonMass() {
  let mesh = horizonPool[horizonActive];
  if (!mesh) {
    mesh = createBox("horizonMallMass", { width: 1, height: 1, depth: 1 }, new BABYLON.Vector3(0, -1000, 0), materials.shadowMat, false);
    mesh.parent = horizonRoot;
    mesh.isPickable = false;
    mesh.applyFog = true;
    horizonPool.push(mesh);
  }
...
function updateHorizon(cx, cz, level) {
  const key = `${cx},${cz},${level}`;
  if (key === horizonKey) return;
  horizonKey = key;
  horizonActive = 0;
  for (let x = cx - horizonRadius; x <= cx + horizonRadius; x++) {
    for (let z = cz - horizonRadius; z <= cz + horizonRadius; z++) {
      const dx = Math.abs(x - cx);
      const dz = Math.abs(z - cz);
      if (dx <= chunkRadius && dz <= chunkRadius) continue;
      if (dx !== horizonRadius && dz !== horizonRadius && seeded(x, z) < 0.52) continue;
      const wx = x * chunkSize + chunkSize / 2;
      const wz = z * chunkSize + chunkSize / 2;
      const r = seeded(x * 13, z * 17);
      const district = districtTheme(x * chunkTiles, z * chunkTiles);
      const mass = acquireHorizonMass();
      mass.position.set(wx, level * levelHeight + 2.1 + r * 0.8, wz);
      mass.scaling.set(chunkSize * (0.55 + r * 0.25), 4.2 + r * 3.6, chunkSize * (0.25 + r * 0.18));
      mass.material = r > 0.82 ? district.light() : materials.shadowMat;
      mass.rotation.y = r > 0.5 ? Math.PI / 2 : 0;
    }
  }
  for (let i = horizonActive; i < horizonPool.length; i++) horizonPool[i].setEnabled(false);
}
...
function updateMall() {
  ...
  pruneChunkJobs(cx, cz, tl);
  updateHorizon(cx, cz, tl);
  processChunkJobs();

  // Collect what's missing, build only the nearest few this frame. Generating a
  // whole ring of chunks in one frame is what causes the hitch when you move; by
  // capping per-frame builds the cost is amortised and movement stays smooth.
  const missing = [];
  for (let x = cx - chunkRadius; x <= cx + chunkRadius; x++) {
    for (let z = cz - chunkRadius; z <= cz + chunkRadius; z++) {
      for (let l = tl - vRadiusDown; l <= tl + vRadiusUp; l++) {
        const key = chunkId(x, z, l);
        const d = (x - cx) ** 2 + (z - cz) ** 2 + ((l - tl) * 1.5) ** 2;
        const lod = d <= nearDetailRadius ? "near" : "mid";
        const existing = mall.get(key);
        if ((!existing || existing._lod !== lod) && !(existing?._lod === "near")) {
          missing.push({ x, z, l, d, lod });
        }
      }
    }
  }
  if (missing.length) {
    ...
    for (let i = 0; i < Math.min(GEN_BUDGET, missing.length); i++) {
      queueChunk(missing[i].x, missing[i].z, missing[i].l, missing[i].lod);
    }
  }
  ...
  for (const [key, items] of mall) {
    ...
    const outOfRange =
      Math.abs(x - cx) > chunkRadius + unloadPadding ||
      Math.abs(z - cz) > chunkRadius + unloadPadding ||
      l > tl + vRadiusUp + 1 ||
      l < tl - vRadiusDown - 1;
    if (!outOfRange) continue;
    unloadChunk(key, items);
  }
  for (let i = placed.length - 1; i >= 0; i--) {
    if (placed[i].isDisposed?.()) placed.splice(i, 1);
  }
}
...
function scavenge() {
  ...
  if (state.focused.metadata.chunkKey && state.focused.metadata.objectKey) {
    getChunkEdits(state.focused.metadata.chunkKey).removed.add(state.focused.metadata.objectKey);
  }
...
function placeBuild() {
  ...
  const chunk = positionToChunk(hit.pickedPoint);
  const key = chunkId(chunk.cx, chunk.cz, chunk.level);
  const items = mall.get(key);
  const levelNode = items?.find((item) => item._isLevelNode);
  const localY = hit.pickedPoint.y - chunk.level * levelHeight;
  if (state.buildMode === "barricade") {
    ...
    state.wood -= 4;
    state.scrap -= 1;
    const build = { type: "barricade", x: hit.pickedPoint.x, y: localY + 1.1, z: hit.pickedPoint.z, rotation: camera.rotation.y };
    getChunkEdits(key).builds.push(build);
    const wall = createBox("barricade", { width: 3.8, height: 2.2, depth: 0.45 }, new BABYLON.Vector3(build.x, build.y, build.z), materials.barricade);
    wall.rotation.y = camera.rotation.y;
    wall.metadata = { chunkKey: key };
    if (levelNode) wall.parent = levelNode;
    if (items) items.push(wall);
    placed.push(wall);
    ...
    state.wood -= 5;
    state.scrap -= 2;
    const build = { type: "camp", x: hit.pickedPoint.x, y: localY + 0.26, z: hit.pickedPoint.z, rotation: 0 };
    getChunkEdits(key).builds.push(build);
    const camp = createCylinder("camp", { diameter: 1.3, height: 0.45 }, new BABYLON.Vector3(build.x, build.y, build.z), materials.camp, false);
    const light = new BABYLON.PointLight("campLight", new BABYLON.Vector3(build.x, build.y + 1.2, build.z), scene);
    light.diffuse = new BABYLON.Color3(1, 0.42, 0.16);
    light.intensity = 1.4;
    light.range = 8;
    camp.metadata = { chunkKey: key };
    if (levelNode) {
      camp.parent = levelNode;
      light.parent = levelNode;
    }
    if (items) items.push(camp, light);
    placed.push(camp, light);
    flash("Camp set up");
  }
  updateUi();
...
function isNearCamp() {
  return placed.some((item) => item.name === "camp" && !item.isDisposed?.() && BABYLON.Vector3.Distance(item.getAbsolutePosition ? item.getAbsolutePosition() : item.position, camera.position) < 7);
}
...
engine.runRenderLoop(() => {
  ...
  hemi.intensity = 0.5 + (state.battery / 100) * 0.12;
  ...
  if (statsUpdateTimer >= 1) {
    statsUpdateTimer = 0;
    const pc = positionToChunk(camera.position);
    window.__mallStats = {
      fps: Math.round(engine.getFps()),
      activeChunks: mall.size,
      queuedChunks: chunkJobs.length,
      meshes: scene.meshes.length,
      lights: scene.lights.length,
      activeMeshes: scene.getActiveMeshes().length,
      totalVertices: scene.getTotalVertices(),
      playerChunk: `${pc.cx},${pc.cz},${pc.level}`,
    };
    ui.debug.innerHTML = [
      `FPS ${window.__mallStats.fps}`,
      `Chunks ${window.__mallStats.activeChunks} (${window.__mallStats.queuedChunks} queued)`,
      `Objects ${window.__mallStats.meshes}`,
      `Active ${window.__mallStats.activeMeshes}`,
      `Chunk ${window.__mallStats.playerChunk}`,
    ].join("<br>");
  }
  scene.render();
});
