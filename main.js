...
function generateChunk(cx, cz, level) {
  ...
  for (let i = root.length - 1; i >= 0; i--) {
    ...
    item.metadata = { ...(item.metadata || {}), chunkKey: key, objectKey };
  ...
  for (const build of edits.builds) rehydrateBuild(root, build, levelNode);
  root.push(levelNode);
...
const GEN_BUDGET = 2; // max chunks built per update — spreads streaming work out

function updateHorizon(cx, cz, level) {
  const key = `${cx},${cz},${level}`;
  if (key === horizonKey) return;
  horizonKey = key;
  horizonRoot.getChildMeshes(false).forEach((mesh) => mesh.dispose());
  for (let x = cx - horizonRadius; x <= cx + horizonRadius; x++) {
    for (let z = cz - horizonRadius; z <= cz + horizonRadius; z++) {
      ...
      const mass = createBox(
        "horizonMallMass",
        { width: chunkSize * (0.55 + r * 0.25), height: 4.2 + r * 3.6, depth: chunkSize * (0.25 + r * 0.18) },
        new BABYLON.Vector3(wx, level * levelHeight + 2.1 + r * 0.8, wz),
        r > 0.82 ? district.light() : materials.shadowMat,
        false,
      );
      mass.rotation.y = r > 0.5 ? Math.PI / 2 : 0;
      mass.parent = horizonRoot;
...
function updateMall() {
  ...
  if (missing.length) {
    ...
    for (let i = 0; i < Math.min(GEN_BUDGET, missing.length); i++) {
      generateChunk(missing[i].x, missing[i].z, missing[i].l);
    }
  }
