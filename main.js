...
scene.fogColor = new BABYLON.Color3(0.045, 0.047, 0.052);
...
material("floor", new BABYLON.Color3(0.47, 0.45, 0.4));
material("marble", new BABYLON.Color3(0.74, 0.7, 0.62), 0.36);
material("tileAlt", new BABYLON.Color3(0.66, 0.64, 0.59), 0.42);
material("grout", new BABYLON.Color3(0.39, 0.36, 0.32), 0.7);
material("floorTrim", new BABYLON.Color3(0.24, 0.19, 0.16), 0.55);
...
material("shopDark", new BABYLON.Color3(0.065, 0.066, 0.069), 0.68);
material("shopGlass", new BABYLON.Color3(0.28, 0.55, 0.62), 0.08);
materials.shopGlass.alpha = 0.32;
material("railGlass", new BABYLON.Color3(0.52, 0.78, 0.82), 0.04);
materials.railGlass.alpha = 0.38;
material("chrome", new BABYLON.Color3(0.88, 0.78, 0.58), 0.18, 0.8);
...
material("plant", new BABYLON.Color3(0.16, 0.5, 0.27));
material("planter", new BABYLON.Color3(0.18, 0.16, 0.14), 0.56);
material("wood", new BABYLON.Color3(0.45, 0.29, 0.16));
material("scrap", new BABYLON.Color3(0.52, 0.54, 0.55), 0.4, 0.2);
...
materials.storeWarm.emissiveColor = new BABYLON.Color3(0.18, 0.1, 0.03);
material("storeCool", new BABYLON.Color3(0.45, 0.72, 0.86), 0.34);
materials.storeCool.emissiveColor = new BABYLON.Color3(0.04, 0.1, 0.14);
material("storeSoft", new BABYLON.Color3(0.78, 0.58, 0.72), 0.42);
materials.storeSoft.emissiveColor = new BABYLON.Color3(0.12, 0.05, 0.1);
...
function makeSign(text, position, rotation = 0) {
  ...
  texture.drawText(text, null, 78, "bold 38px Arial", "#f8e7b5", "#22262a", true);
...
function addPanelText(root, text, position, rotation = 0, width = 2.4, height = 0.9, bg = "#1d2428", fg = "#f3e6bf") {
  ...
  texture.drawText(text, null, 112, "bold 42px Arial", fg, bg, true);
...
function addPlanter(root, x, z, scale = 1) {
  root.push(createBox("planterBox", { width: 2.2 * scale, height: 0.7 * scale, depth: 1.4 * scale }, new BABYLON.Vector3(x, 0.35 * scale, z), materials.planter, false));
  for (let i = 0; i < 5; i++) {
    const leaf = createBox("plantLeaf", { width: 0.22 * scale, height: 0.08 * scale, depth: 1.15 * scale }, new BABYLON.Vector3(x + (i - 2) * 0.28 * scale, 0.9 * scale, z), materials.plant, false);
    leaf.rotation.y = (i - 2) * 0.45;
...
function addTrashBin(root, x, z) {
  root.push(createCylinder("trashBin", { diameter: 0.62, height: 1.05, tessellation: 16 }, new BABYLON.Vector3(x, 0.52, z), materials.blackTrim, false));
  root.push(createCylinder("trashRim", { diameter: 0.68, height: 0.08, tessellation: 16 }, new BABYLON.Vector3(x, 1.08, z), materials.chrome, false));
}

function addBench(root, x, z, rotation = 0) {
  const seat = createBox("benchSeat", { width: 3.2, height: 0.22, depth: 0.8 }, new BABYLON.Vector3(x, 0.62, z), materials.wood, false);
  const back = createBox("benchBack", { width: 3.2, height: 0.85, depth: 0.16 }, new BABYLON.Vector3(x, 1.02, z - 0.42), materials.wood, false);
  const legA = createBox("benchLeg", { width: 0.16, height: 0.55, depth: 0.16 }, new BABYLON.Vector3(x - 1.25, 0.3, z), materials.blackTrim, false);
  const legB = createBox("benchLeg", { width: 0.16, height: 0.55, depth: 0.16 }, new BABYLON.Vector3(x + 1.25, 0.3, z), materials.blackTrim, false);
  for (const mesh of [seat, back, legA, legB]) mesh.rotation.y = rotation;
  root.push(seat, back, legA, legB);
}
...
function addDirectory(root, x, z, rotation = 0) {
  ...
  const board = createBox("directoryBoard", { width: 1.8, height: 2.3, depth: 0.18 }, new BABYLON.Vector3(x, 1.85, z), materials.directory, false);
  post.rotation.y = rotation;
  board.rotation.y = rotation;
  root.push(post, board);
...
function addServiceDoor(root, x, z, rotation = 0) {
  ...
  kick.rotation.y = rotation;
  root.push(frame, door, kick);
...
function addCautionStand(root, x, z, rotation = 0) {
  const sign = createBox("cautionStand", { width: 0.52, height: 0.82, depth: 0.08 }, new BABYLON.Vector3(x, 0.55, z), materials.warningYellow, false);
  sign.rotation.y = rotation;
  root.push(sign);
}

function addSecurityCamera(root, x, z, rotation = 0, y = 4.35) {
  const mount = createBox("cameraMount", { width: 0.12, height: 0.12, depth: 0.36 }, new BABYLON.Vector3(x, y, z), materials.blackTrim, false);
  const body = createCylinder("securityCamera", { diameterTop: 0.26, diameterBottom: 0.2, height: 0.55, tessellation: 12 }, localPoint(x, z, rotation, 0, -0.34, y - 0.05), materials.whiteWall, false);
  const lens = createCylinder("cameraLens", { diameter: 0.14, height: 0.06, tessellation: 12 }, localPoint(x, z, rotation, 0, -0.64, y - 0.05), materials.screen, false);
  for (const mesh of [mount, body, lens]) {
    mesh.rotation.y = rotation;
    mesh.rotation.x = Math.PI / 2;
  }
  mount.rotation.x = 0;
  root.push(mount, body, lens);
}
...
function addVendingMachine(root, x, z, rotation = 0, mat = materials.posterRed) {
  ...
  for (const mesh of [body, glass, slot, toe]) mesh.rotation.y = rotation;
  root.push(body, glass, slot, toe);
}
...
function addATM(root, x, z, rotation = 0) {
  ...
  for (const mesh of [body, screen, keypad, card]) mesh.rotation.y = rotation;
  root.push(body, screen, keypad, card);
}
...
function addMaintenanceCart(root, x, z, rotation = 0) {
  ...
  for (const mesh of [base, bucket, handle, mop]) mesh.rotation.y = rotation;
  handle.rotation.z = 0.35;
  mop.rotation.z = -0.42;
  root.push(base, bucket, handle, mop);
}
...
function addElevatorBank(root, x, z, rotation = 0) {
  ...
  for (const mesh of [wall, left, right, seam, call, header]) mesh.rotation.y = rotation;
  root.push(wall, left, right, seam, call, header);
...
function addSoftSeatingIsland(root, x, z, rotation = 0) {
  ...
  for (const mesh of [rug, seatA, seatB]) mesh.rotation.y = rotation;
  root.push(rug, seatA, seatB);
...
function addEscalatorLanding(root, x, z, angle = 0) {
  ...
  root.push(landing, comb);
}

function addMannequin(root, x, z, rotation = 0) {
  const body = createCylinder("mannequinBody", { diameterTop: 0.5, diameterBottom: 0.7, height: 1.2, tessellation: 12 }, new BABYLON.Vector3(x, 1.05, z), materials.mannequin, false);
  const head = createSphere("mannequinHead", { diameter: 0.38, segments: 12 }, new BABYLON.Vector3(x, 1.85, z), materials.mannequin, false);
  const stand = createCylinder("mannequinStand", { diameter: 0.7, height: 0.06, tessellation: 12 }, new BABYLON.Vector3(x, 0.06, z), materials.chrome, false);
  for (const mesh of [body, head, stand]) mesh.rotation.y = rotation;
  root.push(body, head, stand);
}
...
function addProductShelf(root, x, z, rotation = 0, colorMat = materials.posterBlue) {
  ...
  for (let row = 0; row < 2; row++) {
    for (let col = -1; col <= 1; col++) {
      const box = createBox("productBox", { width: 0.26, height: 0.24, depth: 0.18 }, new BABYLON.Vector3(x + col * 0.34, 0.45 + row * 0.38, z - 0.24), colorMat, false);
      box.rotation.y = rotation;
      root.push(box);
    }
  }
...
function addRailPosts(root, x, z, horizontal = true, y = 4.3) {
  for (let i = -2; i <= 2; i++) {
    const px = horizontal ? x + i * 3 : x;
    const pz = horizontal ? z : z + i * 3;
    root.push(createCylinder("railPost", { diameter: 0.08, height: 1.05, tessellation: 8 }, new BABYLON.Vector3(px, y, pz), materials.chrome, false));
  }
}

function addShopperSilhouette(root, x, z, rotation = 0) {
  const body = createCylinder("shopperBody", { diameterTop: 0.42, diameterBottom: 0.5, height: 1.25, tessellation: 8 }, new BABYLON.Vector3(x, 0.95, z), materials.silhouette, false);
  ...
  for (const mesh of [body, head, bag]) mesh.rotation.y = rotation;
  root.push(body, head, bag);
}
...
function addChromeColumn(root, x, z, y = 3.5, height = 7) {
  root.push(createCylinder("chromeColumn", { diameter: 0.75, height }, new BABYLON.Vector3(x, y, z), materials.chrome));
}
...
function addPalm(root, x, z, scale = 1) {
  const trunk = createCylinder("palmTrunk", { diameter: 0.38 * scale, height: 3.8 * scale }, new BABYLON.Vector3(x, 1.9 * scale, z), materials.wood, false);
  const leaves = [];
  for (let i = 0; i < 7; i++) {
  ...
  for (let i = 0; i < 5; i++) {
    const leaf = createBox("palmFrond", { width: 0.28 * scale, height: 0.12 * scale, depth: 2.6 * scale }, new BABYLON.Vector3(x, 3.7 * scale, z), materials.plant, false);
    leaf.rotation.y = (Math.PI * 2 * i) / 5;
    leaf.rotation.x = Math.PI / 7;
    leaves.push(leaf);
  }
...
function addEscalator(root, x, z, angle = 0) {
  const base = createBox("escalator", { width: 1.8, height: 0.34, depth: 8.5 }, new BABYLON.Vector3(x, 2.35, z), materials.scrap, false);
  const glassL = createBox("escalatorGlass", { width: 0.1, height: 0.7, depth: 8.5 }, new BABYLON.Vector3(x - 1, 2.75, z), materials.railGlass, false);
  const glassR = createBox("escalatorGlass", { width: 0.1, height: 0.7, depth: 8.5 }, new BABYLON.Vector3(x + 1, 2.75, z), materials.railGlass, false);
  const steps = [];
  for (let i = -4; i <= 4; i++) {
    steps.push(createBox("escalatorStep", { width: 1.45, height: 0.045, depth: 0.58 }, new BABYLON.Vector3(x, 2.57 + i * 0.055, z + i * 0.83), materials.blackTrim, false));
  }
  for (const mesh of [base, glassL, glassR]) {
...
function addTableSet(root, x, z, rotation = 0) {
  const table = createCylinder("foodTable", { diameter: 1.1, height: 0.18 }, new BABYLON.Vector3(x, 0.75, z), materials.table, false);
  ...
  root.push(table, stem, foot);
  for (let i = 0; i < 4; i++) {
    const a = rotation + (Math.PI * i) / 2;
    const chair = createBox("foodChair", { width: 0.55, height: 0.22, depth: 0.55 }, new BABYLON.Vector3(x + Math.cos(a) * 1.15, 0.45, z + Math.sin(a) * 1.15), materials.seat, false);
