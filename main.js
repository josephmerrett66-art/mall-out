...
const chunkRadius = 2;
const horizonRadius = chunkRadius + 2;
...
function districtInfo(tileX, tileZ) {
  ...
  const nearOrigin = bx === 0 && bz === 0 && Math.abs(localX) <= 1 && Math.abs(localZ) <= 1;
  const type = nearOrigin ? "atrium" : roll > 0.84 ? "foodCourt" : roll > 0.72 ? "atrium" : "concourse";
